...
import com.zkt.project.biology.service.QueryExceptionSolvedOrderService;
 ...
 */
@RestController
@Api(value="QueryExceptionSolvedOrderController|已处理异常订单")
@RequestMapping(value = "/api/bio/queryExceptionSolvedOrder")
public class QueryExceptionSolvedOrderController {

	private Logger log = LoggerFactory.getLogger(QueryExceptionSolvedOrderController.class);

	@Autowired
	private QueryExceptionSolvedOrderService queryExceptionSolvedOrderService;
	...
	@ApiOperation(value="查询已处理运输异常订单",tags = "查询已处理运输异常订单")
	public ReturnObjectHandle searchProblemmonitor() {
		JSONObject json = ContainerContent.clientWebReceive();
		return queryExceptionSolvedOrderService.searchProblemmonitor(json);
	}
	...
	@ApiOperation(value="查询已处理接收异常订单",tags = "查询已处理接收异常订单")
	public ReturnObjectHandle search() {
		JSONObject json = ContainerContent.clientWebReceive();
		return queryExceptionSolvedOrderService.search(json);
	}
	...
	@ApiOperation(value="已处理异常订单详情",tags = "已处理异常订单详情")
	public ReturnSimpleHandle detail() {
		JSONObject json = ContainerContent.clientWebReceive();
		return queryExceptionSolvedOrderService.detail(json);
	}
	...
	public void export() {
		...
		try {
			...
			ReturnObjectHandle handle = queryExceptionSolvedOrderService.search(infoList);
	...
	public void exportProblemmonitor() {
		...
		try {
			...
			JSONObject infoList = json.getJSONObject("infoList");
			infoList.put("isExport", 'Y');
			ReturnObjectHandle handle = queryExceptionSolvedOrderService.searchProblemmonitor(infoList);
