...
import com.zkt.project.biology.service.ReportSenderCityAreaService;
 ...
 */
@RestController
@Api(value="ReportSenderAreaController|各市区医院报表")
@RequestMapping(value = "/api/bio/reportSenderCityArea")
public class ReportSenderCityAreaController {

	private Logger log = LoggerFactory.getLogger(ReportSenderCityAreaController.class);

	@Autowired
	private ReportSenderCityAreaService reportSenderCityAreaService;
	...
	public ReturnObjectHandle search() {
		...
		return reportSenderCityAreaService.search(json);
	}
	...
	public ReturnSimpleHandle detail() {
		JSONObject json = ContainerContent.clientWebReceive();
		return reportSenderCityAreaService.detail(json);
	}
	...
	public void exportpass() {
		...
		try {
			...
			ReturnObjectHandle handle = reportSenderCityAreaService.search(infoList);
