...
import com.zkt.project.biology.service.ReportSenderService;
 ...
 */
@RestController
@Api(value="ReportSenderController|各医院发货报表")
@RequestMapping(value = "/api/bio/reportSender")
public class ReportSenderController {

	private Logger log = LoggerFactory.getLogger(ReportSenderController.class);
	...
	public ReturnObjectHandle search() {
		...
		return reportSenderService.search(json);
	}
	...
	public ReturnSimpleHandle detail() {
		JSONObject json = ContainerContent.clientWebReceive();
		return reportSenderService.detail(json);
	}
	...
	public void exportpass() {
		...
		try {
			...
			ReturnObjectHandle handle = reportSenderService.search(infoList);
