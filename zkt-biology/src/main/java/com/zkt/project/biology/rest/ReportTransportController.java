...
import com.zkt.project.biology.service.ReportTransportService;
...
public class ReportTransportController {
	...
	@Autowired
	private ReportTransportService reportTransportService;
	...
	public ReturnObjectHandle search(@ApiParam(name="transport",value="运输公司名称") String transport,
		...
		return reportTransportService.search(json);
	}
	...
	public ReturnSimpleHandle detail(@ApiParam(name="orderNo",value="订单编号",required=true) String orderNo) {
		...
		JSONObject json = JSONObject.fromObject( map );
		return reportTransportService.detail(json);
	}
	...
	public void exportpass(@ApiParam(name="transport",value="运输公司名称") String transport,
			@ApiParam(name="startDate",value="开始时间") String startDate,
			@ApiParam(name="endDate",value="结束时间") String endDate,
			@ApiParam(name="orderStatus",value="订单状态") String orderStatus,
			@ApiParam(name="isProblem",value="异常 0：是异常件 1：不是异常件") String isProblem,
		...
		try {
			...
			ReturnObjectHandle handle = reportTransportService.search(json);
