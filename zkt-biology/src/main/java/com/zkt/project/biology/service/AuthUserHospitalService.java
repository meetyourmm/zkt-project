...
@Service
public class AuthUserHospitalService {
	...
	public ReturnObjectHandle search(JSONObject json){
		...
		if (Constant.USER_TYPE_BOSS.equals(userType)) {
			map.put("userType", Constant.USER_TYPE_EMPLOY);		
		} 
	...
	public ReturnSimpleHandle saveServer(JSONObject json){
		...
		logins.setUserType(Constant.USER_TYPE_EMPLOY);// 用户类型
		if(userType.equals(Constant.USER_TYPE_BOSS)){
			logins.setRoleId(json.getString("hospital"));// 关联上级ID
		}else{
