...
@Service
public class AuthUserTransportService {
	...
	public ReturnObjectHandle search(JSONObject json){
		...
		if (Constant.USER_TYPE_BOSS.equals(userType)) {
			map.put("userType", Constant.USER_TYPE_TRANSPORTEMPLOY);		
		} 
		
		//运输方查看各自对应的员工
		if(Constant.USER_TYPE_TRANSPORT.equals(userType)){
			map.put("userType", Constant.USER_TYPE_TRANSPORTEMPLOY);
			map.put("roleId", userId);//通过userType roleId 锁定运输方所属员工
		}
	...
	public ReturnSimpleHandle saveServer(JSONObject json){
		...
		logins.setUserType(Constant.USER_TYPE_TRANSPORTEMPLOY);// 用户类型
