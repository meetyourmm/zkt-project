...
@Service
public class WeChatAbnormalService {
	...
	public String abnormal(HttpServletRequest request) throws Exception {
		...
		if (ValidateUtil.isEmpty(userName) || ValidateUtil.isEmpty(orderNo) || ValidateUtil.isEmpty(cageno)
				 || ValidateUtil.isEmpty(discribtion)) {
		...
		log.warn("discribtion" + discribtion);
		...
		if (order.getOrderStatus().equals(Constant.ORDER_STATUS_XIETIAO)) {
			result.put(RequestConstance.RESULT_CODE, RequestTag.CODE_FAIL);
			result.put(RequestConstance.RESULT_MSG, "不能重复提交异常!");
			return result.toString();
		}
		...
		order.setProblemCause(discribtion);
		...
		if(null != wechatusers && wechatusers.getUserhospitalid().equals(senderid) && wechatusers.getUsertype().equals(Constant.USER_TYPE_HOSPITAL)){			
			HttpRequest.sendPost("http://m.jerbio.com/index.php/Z017/Message/refuse_inform", "orderno="+orderNo+"&receiver="+order.getReciver()+"&openid="+wechatusers.getUserid()+"&discribtion="+discribtion+"&operator="+userName);
		}
